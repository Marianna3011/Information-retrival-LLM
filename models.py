...
def answer_question(contex: str, question: str) -> Tuple[str, float]:
    '''
    ...
    best_score = Evaluate(question, predicted_answer, context)
    try:
        flan_prompt = f'Based on the context: {truncated_context}; generate an answer to the question: {question}'
        predicted_answer_flan = flan(flan_prompt, max_new_tokens=128)[0]['generated_text']
    except Exception as e:
        predicted_answer_flan = 'CANNOTANSWER'
    score_flan = Evaluate(question, predicted_answer_flan, context)
    if score_flan > best_score:
        best_answer = predicted_answer_flan
        best_score = score_flan
    try:
        bert_score = bert_pipeline(question=question, context=context)
        predicted_answer_bert = bert_score['answer']
    except Exception as e:
        predicted_answer_bert = 'CANNOTANSWER'
    score_bert = Evaluate(question, predicted_answer_bert, context)
